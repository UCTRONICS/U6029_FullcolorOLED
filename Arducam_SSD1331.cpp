/*************************************************** 
  ...
  interface
  Arducam invests time and resources providing this open source code, 
  please support Arducam and open-source hardware by purchasing 
  products from Arducam!

 ****************************************************/

#include "Adafruit_GFX.h"
#include "Arducam_SSD1331.h"
#include "glcdfont.c"
...
inline void Arducam_SSD1331::spiwrite(uint8_t c) {
...
void Arducam_SSD1331::writeCommand(uint8_t c) {
    *rsportreg &= ~ rspin;
...
void Arducam_SSD1331::writeData(uint8_t c) {
    *rsportreg |= rspin;
...
void Arducam_SSD1331::goHome(void) {
  goTo(0,0);
}

void Arducam_SSD1331::goTo(int x, int y) {
...
uint16_t Arducam_SSD1331::Color565(uint8_t r, uint8_t g, uint8_t b) {
  uint16_t c;
  c = r >> 3;
  c <<= 6;
  c |= g >> 2;
...
/**************************************************************************/
/*
void Arducam_SSD1331::fillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t fillcolor) 
{
...
void Arducam_SSD1331::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) {	
...
void Arducam_SSD1331::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void Arducam_SSD1331::pushColor(uint16_t color) {
  // setup for data
  *rsportreg |= rspin;
  *csportreg &= ~ cspin;
...
void Arducam_SSD1331::begin(void) {
    // set pin directions
    pinMode(_rs, OUTPUT);
...
Arducam_SSD1331::Arducam_SSD1331(uint8_t cs, uint8_t rs, uint8_t sid, uint8_t sclk, uint8_t rst) : Adafruit_GFX(TFTWIDTH, TFTHEIGHT) {
    _cs = cs;
    _rs = rs;
    _sid = sid;
    _sclk = sclk;
...
Arducam_SSD1331::Arducam_SSD1331(uint8_t cs, uint8_t rs, uint8_t rst) : Adafruit_GFX(TFTWIDTH, TFTHEIGHT) {
    _cs = cs;
    _rs = rs;
    _sid = 0;
    _sclk = 0;
